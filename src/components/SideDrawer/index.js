...
export default function SideDrawer({ open, setOpen, setSharedCategoryState }) {
    ...
    const [addCategoryButtonColor, setAddCategoryButtonColor] =
        React.useState(lightGrey);
    const [categories, setCategories] = React.useState(
        localStorage.getItem("categories").split(",")
    );
    ...
    const drawerItems = () => (
        <Box sx={{ width: 250 }} role="presentation">
            <Typography sx={{ margin: "20px 0 10px 10px", fontSize: "1.5rem" }}>
                Categories
            </Typography>
            ...
            <List>
                {categories.map((text, index) =>
                    editState.inEditMode && editState.index === index ? (
                        <TextField
                            onChange={(e) =>
                                setEditedCategoryText(e.target.value)
                            }
